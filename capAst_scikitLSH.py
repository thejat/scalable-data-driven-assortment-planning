...
def capAst_NNalgo(prod, C, p, v,  eps,  algo = 'exactNN', nEst =10, nCand =40 , preprocessed =False, KList=None, dbList=None, normConstList=None):
    ...
    while (U - L) >eps:
        ...
        maxPseudoRev, maxSet= calcNN(v,p,K, prod, C, KList, dbList, normConstList)
...
def calcNN(v,p,K, prod, C, KList, dbList, normConstList):
    prec = 10**(-5)
    ...
    #idx = np.where(np.around(KList, decimals =5) == np.around(K, decimals =5))
    query = np.concatenate((v[1:], [0])) #appending extra coordinate as recommended by Simple LSH
    #print 'K is ', K
